...
import CartaoFoto from '../CartaoFoto/CartaoFoto'
...
class UtilitiesColors extends React.Component {
    ...
    preencherUrl() {
        ...
        let novo = this.state.items.map((item, idx) => {
            return Object.assign(item, {url: data});
        })
                    ...
                    <Row>
                        ...
                        <Col md="12">
                            <Card className="main-card mb-3">
                                <CardBody>
                                    <CardTitle>Fotinhos</CardTitle>

                                    {this.state.items.length === 0 && ( <Row style={{justifyContent: 'center'}}> <Col xs={"auto"}>Não há registros </Col> </Row> ) }

                                    <Row>

                                        {this.state.items.filter(item => item.name.toUpperCase().includes(this.props.valorBusca.toUpperCase()))
                                            .map((item, idx) => {
                                            return (<CartaoFoto key={item.id} dados={item}>  </CartaoFoto>)
                                        })}
